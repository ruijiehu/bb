  ...
  onLoad: function (options) {
    ...
    wx.getSystemInfo({
      success: function (res) {
        console.info(res)
        console.info(res.SDKVersion)
        SDKVersion = res.SDKVersion
        that.setData({
          resgit_bg: res.windowHeight
        });
      }
    })
    console.info(SDKVersion)
    if(that.compareVersion(SDKVersion, '1.9.90')==1){
      // 此手机版本比1.9.90高
      //强制更新小程序
    ...
    } else if (that.compareVersion(SDKVersion, '1.9.90') == -1){
       // 此手机版本比1.9.90低
      //  不支持强制更新
    }
 ...
 */
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.success) {
          ...
          //   that.setData({
          //     ListAll: that.data.ListAll.splice(10, 0, '454545')
          //   })
  ...
  getAll() {
    ...
    if (that.data.listHasMore) {
      wx.request({
        url: app.globalData.javahost + '/user/operate/get/index/page',
        method: 'POST',
        data: reqdata_new,
        header: {
          'content-type': 'application/json',
        ...
        success: function (res) {
          ...
          if (res.data.success) {
            ...
            for (let i = 0; i < res.data.data.list.length; i++) {
              ...
              if (cache.type == 0) {
                // cache.newsUView.images = cache.newsUView.images ? cache.newsUView.images.split(";") : [];
              } else if (cache.type == 4) {
                //转发
                if (cache.fowardShareIndexUView.shareListUView.type == 1) {
                  cache.fowardShareIndexUView.shareListUView.questionUView.images = cache.fowardShareIndexUView.shareListUView.questionUView.images ? cache.fowardShareIndexUView.shareListUView.questionUView.images.split(";") : [];
                } else if (cache.fowardShareIndexUView.shareListUView.type == 2) {
                  // cache.fowardShareIndexUView.shareListUView.purchaseUView.images = cache.fowardShareIndexUView.shareListUView.purchaseUView.images ? cache.shareListUView.purchaseUView.images.split(";") : [];
                } else if (cache.fowardShareIndexUView.shareListUView.type == 0) {
                  // cache.fowardShareIndexUView.shareListUView.newsUView.images = cache.fowardShareIndexUView.shareListUView.newsUView.images ? cache.shareListUView.newsUView.images.split(";") : [];
                }
              } else if (cache.type == 2) {
                cache.purchaseDetailUView.images = cache.purchaseDetailUView.images ? cache.purchaseDetailUView.images.split(";") : [];
              } else if (cache.type == 3) {
                cache.questionUView.images = cache.questionUView.images ? cache.questionUView.images.split(";") : [];
              } else if (cache.type == 1) {
                if (cache.shareListUView.type == 0) {
                  // 分享精选
                  //   cache.shareListUView.newsUView.images = cache.shareListUView.newsUView.images ? cache.shareListUView.newsUView.images.split(";") : [];
                  // } else if (cache.shareListUView.type == 1){
                  // 分享采购
                  // cache.shareListUView.purchaseUView.images = cache.shareListUView.purchaseUView.images ? cache.shareListUView.purchaseUView.images.split(";") : [];
                  // } else if (cache.shareListUView.type == 2){
                  // 分享交流
                  // cache.shareListUView.questionUView.images = cache.shareListUView.questionUView.images ? cache.shareListUView.questionUView.images.split(";") : [];
                }
              }
 ...
 */
  ...
  to_all () {
    wx.navigateTo({
      url: '/pages/home_page/toutiao_list/list',
    })
  },
  /**
   * 获取推荐的精选
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.success) {
          ...
          console.info(that.data.newsList)
