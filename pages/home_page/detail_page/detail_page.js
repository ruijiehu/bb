...
Page({
  ...
  onLoad: function (options) {
    ...
    console.info(options.newsId+'8888888888888')
    //调用应用实例的方法获取全局数据
  ...
  onShareAppMessage: function () {
    let shareData = {
      ...
      path: '/pages/home_page/detail_page/detail_page?newsId=' + this.data.newsId + '&home=1'
    };
    return shareData;
  },
  /**
  ...
  */
  ...
  getAllComment: function () {
    ...
    // if (that.data.listHasMore) {
      var answerList = [];
      wx.request({
        url: app.globalData.javahost + '/user/news/comment/list',
        method: 'POST',
        // data: reqData,
        data: { "newsId": that.data.newsId},
        header: {
...
*/
  ...
  answerAdd: function (e) {
    let that = this;
      wx.navigateTo({
        url: '/pages/home_page/reply/reply?id=' + that.data.newsId + '&type=0'
      })
  },
  to_home() {
    wx.switchTab({
