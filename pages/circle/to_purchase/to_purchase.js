...
Page({
  ...
  data: {
    ...
    showLoading: true,
    hiddenCircle: true,
    info: { "title": '', "content": '', "number": '', "price": '', "country": '' },
    circleIds: [],
    questionId: '',
    timearray: ['三天', '一周', '一个月'],
    timeIndex: 0,
 ...
 */
  ...
  changebox(e) {
    ...
    //     isc: false
    //   })
    // } else if (!e.currentTarget.dataset.checked){
    //   this.setData({
    //     isc: true
    //   })
    // }
  ...
  formSubmit: function (e) {
    ...
    //   })
    // } else {
    console.info(that.data.isc)
    if (e.detail.value.content && e.detail.value.number) {
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.success) {
            ...
            // wx.navigateTo({
            //   url: '/pages/circle/pur_show/pur_show?id=' + id
            // })
            wx.switchTab({
