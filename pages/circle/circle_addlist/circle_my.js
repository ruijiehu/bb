      ...
      success: function (res) {
        if (res.data.success) {
          ...
          for (let i = 0; i < res.data.data.length; i++) {
            if (res.data.data[i].newMomentViews) {
              res.data.data[i].images = [];
              for (let j = 0; j < res.data.data[i].newMomentViews.length; j++) {
                cache = res.data.data[i].newMomentViews[j];
                cache.images = cache.images ? cache.images.split(";") : [];
                res.data.data[i].images.push(cache.images[0]);
              }
            }
            joinList.push(res.data.data[i]);
            circleIDS.push(joinList[i].circleId)
          }
          console.info(joinList)
          console.info(circleIDS)
          that.setData({
            joinList: joinList
          })
          setTimeout(function () {
