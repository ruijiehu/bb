  ...
  getNewMoment: function () {
    ...
    wx.request({
      ...
      data: {
        "currentPage": 1,
        "pageSize": 3,
        "conditons": [{ "field": "user.userId", "value": that.data.comid }]
      },
      header: {
        'content-type': 'application/json',
        'cookie': 'JSESSIONID=' + app.globalData.session
...
添加足迹 */
  ...
  goGoodsList: function (e) {
    ...
    if (this.data.info.verifyYn) {
      wx.navigateTo({
        url: '/pages/my/goodsList?userId=' + id
      })
    } else {
      if (this.data.verifyApplying) {
        wx.showModal({
 ...
 */
  guanzhu(e) {
    ...
    if (this.data.info.verifyYn) {
      ...
      if (this.data.info.verifyApplying) {
        wx.showModal({
          content: '认证还未通过审核,请耐心等待',
          showCancel: false,
          confirmText: "确定"
